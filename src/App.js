...
const Icon = React.memo(({ family, name, ...props }) => (
  <span style={{ fontFamily: family }} {...props}>
    {!isNaN(IconFamilies[family][name])?String.fromCodePoint(IconFamilies[family][name]):''}
  </span>
));
  ...
  return (
    <div className="Header-Container">
      <div className="Header-Content">
        <h1 className="Header-Title">REACT NATIVE VECTOR ICON DIRECTORY (by THANHVD)</h1>
      </div>
    </div>
  );
};
...
const getMatches = query =>
  {
    return Object.keys(IconFamilies)
    .sort()
    .map(family => {
